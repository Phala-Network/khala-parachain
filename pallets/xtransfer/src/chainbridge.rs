...
pub mod pallet {
        ...
        T::AccountId: Into<[u8; 32]> + From<[u8; 32]>,
    {
        ...
        fn extract_fungible(asset &MultiAsset) -> Option<(MultiLocation, U256)> {
            return match asset {
                (Fungible(&amount), Concrete(location)) => {
                    Some((location, amount as U256))
                },
                _ => None,
            }
        }
	...
	impl<T: Config> BridgeTransact for Pallet<T>
	{
		...
		fn transfer_fungible(
            ...
            let (location, amount) = Self::extract_fungible(&asset).ok_or(Error::<T>::ExtractAssetFailed)?;
            let (dest_id, recipient) = Self::extract_dest(&dest).ok_or(Error::<T>::ExtractDestFailed)?;
            let resource_id = location.into_rid(dest_id);

			log::trace!(
				target: LOG_TARGET,
				" Chainbridge fungible transfer, dest_id: {:?}, resource_id: {:?}, to: {:?}, amount: {:?}.",
				dest_id,
				&resource_id,
				&to,
				amount,
            ...
            // TODO: adapter handle assets transfer
