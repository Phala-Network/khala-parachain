...
const query = gql`
{
  basePoolsConnection(orderBy: id_ASC, first: 10000, where: {kind_eq: Vault, withdrawingShares_not_eq: "0"}) {
    edges {
      node {
        id
      }
...
//     (pid: any) => {
//       return [
//         api.tx.phalaVault.checkAndMaybeForceWithdraw(pid),
//       ]
//     }
//   ).flat()
// );

// console.info(`Sending phalaVault.checkAndMaybeForceWithdraw(pid)`);
...
for (const pid of pids) {
  console.info(`Sending phalaVault.checkAndMaybeForceWithdraw(${pid})`);
  const txPromise = api.tx.phalaVault.checkAndMaybeForceWithdraw(pid);
  console.info(`Call hash: ${txPromise.toHex()}`);
  const txHash = await txPromise.signAndSend(operatorKeyPair, { nonce: -1 });
  console.info(`Transaction hash: ${txHash.toHex()}`);
}
